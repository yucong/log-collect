...
(function (factory) {
    ...
    defaultFileActionSettings = {
        ...
        dragSettings: {},
        indicatorNew: '<i class="glyphicon glyphicon-hand-down text-warning">待上传</i>',
        indicatorSuccess: '<i class="glyphicon glyphicon-ok-sign text-success">上传成功</i>',
        indicatorError: '<i class="glyphicon glyphicon-exclamation-sign text-danger">上传失败</i>',
    ...
    FileInput.prototype = {
        ...
        _initUploadSuccess: function (out, $thumb, allFiles) {
            ...
            /******************自定义开始 上传成功后，自动添加到视图，方便删除****************/
            if(out.data && !$.isEmptyObject(self.multipleSettings)){
                var img_html = "<img src='"+out.data.path+"' class='file-preview-image' ";
                var setting = self.multipleSettings;
                if(setting.deleteUrl && setting.deleteUrl.length>0){
                    var newDeleteUrl = setting.deleteUrl.replace('#path#', out.data.path);
                    out.initialPreviewConfig=[{url: newDeleteUrl}];
                }
                //img_html += "id='"+out.data.id+"' ";
                img_html += "name='"+setting.name+"' ";
                img_html += " />";
                out.initialPreview=[img_html];
                out.append = true;
            }
            /******************自定义结束 上传成功后，自动添加到视图，方便删除****************/
